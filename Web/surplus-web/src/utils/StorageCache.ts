...
export function get(key) {
  ...
  if (storedItem) {
    ...
    var expirationDate = Date.parse(
      arrayOfSplitByPipe[arrayOfSplitByPipe.length - 1]
    );
    ...
    try {
      if (valueObj) {
        window.localStorage.setItem(
          key,
          JSON.stringify(valueObj) + "|" + expirationDate
        );
      }
    } catch {
...
export function ifAbsentSet(key, expirationMinutes, setValue) {
  ...
  try {
    if (setValue) {
      window.localStorage.setItem(
        key,
        JSON.stringify(setValue) + "|" + expirationDate
      );
    }
  } catch {
    window.localStorage.clear();
